...
const SCORING = {
  ...
  scoreEmail(text, prompt) {
    ...
    else if (taskScore === 2) feedback.push({ type: 'ok', text: '⚠️ Task completion: 2 of 3 required points detected. Review the prompt carefully.' });
    ...
    else if (wc >= 110) { wcScore = 1; feedback.push({ type: 'ok', text: '⚠️ Length: ' + wc + ' words — slightly ' + (wc < 130 ? 'short, aim for 130+.' : 'long, aim for 130–180.') }); }
    ...
    else if (politeScore >= 0.5) feedback.push({ type: 'ok', text: '⚠️ Politeness: Decent — add formal phrases like "I would be grateful if…" or "Could you please…"' });
    ...
    else feedback.push({ type: 'ok', text: '⚠️ Vocabulary: Try using more formal expressions (e.g. "I would like to bring to your attention", "I appreciate your prompt response").' });
    ...
    else if (grammar.issues.length === 0) feedback.push({ type: 'ok', text: '⚠️ Grammar: No major errors, but try adding more complex structures (conditionals, passive voice, perfect tenses).' });
    ...
    else feedback.push({ type: 'ok', text: '⚠️ Cohesion: Add more linking phrases (e.g. "Furthermore…", "As a result…", "In addition…").' });
  ...
  scoreDiscussion(text, prompt) {
    ...
    else if (wc >= 90) { wcScore = 1; feedback.push({ type: 'ok', text: '⚠️ Length: ' + wc + ' words — aim for at least 120.' }); }
    ...
    else if (topicScore >= 1) feedback.push({ type: 'ok', text: '⚠️ Relevance: Somewhat on-topic. Make sure you directly address the professor's question.' });
    ...
    if (hasOpinion && hasExample && hasReasoning) feedback.push({ type: 'good', text: '✅ Argument: Strong — clear opinion, supporting example, and reasoning all present.' });
      ...
      if (missing.length) feedback.push({ type: missing.length >= 2 ? 'bad' : 'ok', text: (missing.length >= 2?'❌':'⚠️') + ' Argument: Missing ' + missing.join(' and ') + '.' });
    }
    ...
    else feedback.push({ type: 'ok', text: '⚠️ Engagement: Try referencing your classmates by name (e.g. "While ' + (prompt.students?.[0]?.name||'your classmate') + ' argues…, I believe…").' });
    ...
    else if (langScore >= 1) feedback.push({ type: 'ok', text: '⚠️ Language: Adequate — try incorporating more academic vocabulary and varied sentence types.' });
    ...
    else feedback.push({ type: 'ok', text: '⚠️ Cohesion: Use more varied linking expressions (contrast: "However…"; addition: "Moreover…"; cause: "Therefore…").' });
  ...
  getBand(score) {
    ...
    return { label: 'Needs Development', color: '#ef4444', emoji: '✏️' };
  },
...
function finishTask(){
  ...
  sum.innerHTML='<div class="score-summary"><h2>✅ Task Complete!</h2>'+(currentTask==='build-a-sentence'?'<div class="score-circle" style="background:conic-gradient(var(--primary) '+Math.round(correct/items.length*100)+'%,var(--surface2) 0%);"><div class="score-inner"><span class="score-num">'+correct+'/'+items.length+'</span><span class="score-label">Correct</span></div></div>':'<p style="font-size:1.05rem;margin-bottom:16px;">Your responses have been scored. Review your feedback above or compare with the sample answers.</p>')+'<div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-top:20px;"><button class="btn btn-secondary" onclick="backToTasks()">← Try Another Task</button><a href="mock-test.html" class="btn btn-primary">🎯 Full Mock Test</a></div></div>';
  showVideoAd(()=>{});
}
