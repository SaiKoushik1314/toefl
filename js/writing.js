/* =============================================
   ...
   let timerInterval = null, timeLeft = 0, results = [];
   ...
   const SCORING = {
     ...
     analyzeVocabulary(text) {
       ...
       return {
         ...
         academicCount: academicWords.filter(w => text.toLowerCase().includes(w)).length,
         formalCount: formalWords.filter(w => text.toLowerCase().includes(w)).length,
         weakCount: weakWords.filter(w => text.toLowerCase().includes(w)).length,
     ...
     analyzeCoherence(text) {
       const devices = {
         ...
         contrast: ['however','nevertheless','on the other hand','in contrast','although','despite','while','whereas','yet'],
         ...
         emphasis: ['indeed','in fact','clearly','importantly'],
         concession: ['admittedly','granted','of course','even though']
       };
       let total = 0;
       const found = {};
       for (const [type, words] of Object.entries(devices)) {
         found[type] = words.filter(w => text.toLowerCase().includes(w)).length;
         total += found[type];
       }
     ...
     scoreEmail(text, prompt) {
       ...
       else if (pointsCovered === 2) feedback.push({ type: 'ok', text: 'Task completion: 2 of 3 required points detected. Review the prompt carefully.' });
       else feedback.push({ type: 'bad', text: 'Task completion: Only ' + pointsCovered + ' required point(s) detected. You must address all 3.' });
   
       // Word count (0-2)
       let wcScore = wc >= 130 && wc <= 180 ? 2 : wc >= 110 ? 1 : 0;
       breakdown.length = wcScore;
       if (wc >= 130 && wc <= 180) feedback.push({ type: 'good', text: 'Length: ' + wc + ' words - excellent range.' });
       else if (wc >= 110) feedback.push({ type: 'ok', text: 'Length: ' + wc + ' words - aim for 130+.' });
       ...
       else feedback.push({ type: 'ok', text: 'Vocabulary: Try using more formal expressions.' });
       ...
       else if (grammar.issues.length === 0) feedback.push({ type: 'ok', text: 'Grammar: No major errors, but try adding more complex structures.' });
       else feedback.push({ type: 'bad', text: 'Grammar: Possible issues: ' + grammar.issues.join(', ') + '.' });
       ...
       else feedback.push({ type: 'ok', text: 'Cohesion: Add linking phrases (e.g. "Furthermore...", "As a result...", "In addition...").' });
     ...
     scoreDiscussion(text, prompt) {
       ...
       // Word count (0-2)
       let wcScore = wc >= 150 ? 2 : wc >= 120 ? 1.5 : wc >= 90 ? 1 : 0;
       ...
       else if (wc >= 90) feedback.push({ type: 'ok', text: 'Length: ' + wc + ' words - aim for at least 120.' });
       else feedback.push({ type: 'bad', text: 'Length: ' + wc + ' words - too short. Aim for 120+ words.' });
       ...
       if (topicScore >= 2) feedback.push({ type: 'good', text: 'Relevance: Strong - your response addresses the topic directly.' });
       ...
       const hasExample = /\b(for example|for instance|such as|to illustrate|a study|research shows|evidence suggests|personally|in my experience|consider the case)\b/i.test(text);
       ...
       if (hasOpinion && hasExample && hasReasoning) {
         feedback.push({ type: 'good', text: 'Argument: Strong - clear opinion, supporting example, and reasoning all present.' });
       } else {
         const missing = [];
         ...
         if (missing.length) feedback.push({ type: missing.length >= 2 ? 'bad' : 'ok', text: 'Argument: Missing ' + missing.join(' and ') + '.' });
       }
   
       // Engagement with classmates (0-1)
       const studentNames = (prompt.students || []).map(s => s.name.toLowerCase());
       const engages = studentNames.some(n => text.toLowerCase().includes(n));
       const engageWords = ['agree with', 'disagree with', 'builds on', 'unlike', 'as mentioned', 'point raised', 'argument made'];
       const engagesPhrases = engageWords.some(w => text.toLowerCase().includes(w));
       breakdown.engagement = (engages || engagesPhrases) ? 1 : 0;
       if (breakdown.engagement) feedback.push({ type: 'good', text: "Engagement: You referenced your classmates' contributions - excellent." });
       ...
       else if (langScore >= 1) feedback.push({ type: 'ok', text: 'Language: Adequate - try incorporating more academic vocabulary.' });
     ...
     getBand(score) {
       if (score >= 9) return { label: 'Advanced', color: '#059669', emoji: 'Advanced' };
       if (score >= 7) return { label: 'Upper Intermediate', color: '#10b981', emoji: 'Upper Intermediate' };
       if (score >= 5) return { label: 'Intermediate', color: '#f59e0b', emoji: 'Intermediate' };
       if (score >= 3) return { label: 'Lower Intermediate', color: '#f97316', emoji: 'Lower Intermediate' };
       return { label: 'Needs Development', color: '#ef4444', emoji: 'Needs Development' };
     },
     ...
     renderFeedback(result, type) {
       ...
       const breakdownHtml = Object.entries(breakdown).map(([k, v]) => {
         ...
         return '<div style="margin-bottom:8px;">' +
           '<div style="display:flex;justify-content:space-between;margin-bottom:3px;font-size:0.83rem;">' +
           '<span>' + (breakdownLabels[k] || k) + '</span>' +
           '<span style="font-weight:700;color:' + color + '">' + v + '/' + max + '</span>' +
           '</div>' +
           '<div style="background:#e2e8f0;border-radius:99px;height:6px;overflow:hidden;">' +
           '<div style="background:' + color + ';height:100%;width:' + pct + '%;border-radius:99px;transition:width 0.4s;"></div>' +
           '</div></div>';
       }).join('');
       ...
       return '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:20px;margin-top:16px;">' +
         ...
         '</div></div>' +
         '<div>' +
         '<div style="font-size:1.1rem;font-weight:700;">' + band.label + '</div>' +
   ...
   function showVideoAd(cb) {
     ...
     const t = setInterval(() => {
       s--;
       document.getElementById('adCountdown').textContent = s;
       if (s <= 5) { skip.classList.add('visible'); if (closeX) closeX.classList.add('visible'); }
   ...
   function showIntroModal(data) {
     ...
     const descs = {
       'build-a-sentence': 'Drag and drop words into the correct order. One word may be extra and should not be used.',
   ...
   function startTask() {
     ...
     if (!document.getElementById('backToTasksBtn')) {
       ...
       b.id = 'backToTasksBtn';
       b.className = 'back-to-tasks-btn';
       b.textContent = 'Back to Tasks';
       b.onclick = backToTasks;
   ...
   function backToTasks() {
     ...
     const rc = document.getElementById('rubricsCard');
     if (rc) rc.style.display = 'none';
     const b = document.getElementById('backToTasksBtn');
     if (b) b.remove();
   }
   ...
   function renderBuildSentence(area, q) {
     area.innerHTML =
       ...
       '<div class="drop-zone" id="dropZone" ondragover="allowDrop(event)" ondrop="dropWord(event)"></div>' +
       (q.fixedEnd ? '<p style="margin-bottom:16px;"><strong>' + q.fixedEnd + '</strong></p>' : '') +
       '<p style="font-size:0.85rem;color:var(--text-muted);margin-bottom:10px;">Word Bank - click or drag words into the box above:</p>' +
     ...
     const remaining = shuffle([...q.words]).filter(w => !placed.includes(w));
     ...
     if (saved?.checked) {
       ...
       fb.innerHTML = saved.correct
         ? '<h4>Perfect!</h4><p>' + q.fullSentence + '</p>'
   ...
   function createChip(word) {
     ...
     span.addEventListener('click', () => moveChip(span));
     return span;
   }
   ...
   function moveChip(chip) {
     ...
     if (chip.parentElement === bank) {
       ...
       if (zone.children.length === 0) zone.innerHTML = '<span id="dropHint" style="color:var(--text-light);font-size:0.88rem;">Click words or drag them here...</span>';
     }
   }
   ...
   function checkSentence(idx) {
     const q = items[idx];
     const placed = [...document.getElementById('dropZone').querySelectorAll('.word-chip')].map(c => c.textContent);
     ...
     fb.innerHTML = correct
       ? '<h4>Perfect!</h4><p>' + q.fullSentence + '</p>'
       : '<h4>Not quite.</h4><p>Correct: <strong>' + q.fullSentence + '</strong></p>' +
         '<p style="margin-top:6px;font-size:0.88rem;color:var(--text-muted);">Your answer: ' + q.fixedStart + ' ' + placed.join(' ') + ' ' + q.fixedEnd + '</p>';
   }
   ...
   function renderWriteEmail(area, prompt) {
     ...
     area.innerHTML =
       ...
       '<div class="word-count-bar"><span>Words: <span class="word-count-num" id="wc">0</span></span>' +
       '<span id="wcTarget" style="font-size:0.83rem;">Target: 130-140 words</span></div>' +
       '<div style="display:flex;gap:10px;margin-top:16px;flex-wrap:wrap;">' +
   ...
   function scoreEmail(idx) {
     ...
     if (text.trim().split(/\s+/).filter(Boolean).length < 20) {
       document.getElementById('emailFeedbackArea').innerHTML = '<div style="background:#fee2e2;border-left:4px solid #ef4444;border-radius:8px;padding:16px;margin-top:12px;font-size:0.9rem;">Please write your email first before requesting feedback.</div>';
       return;
     }
   ...
   function renderDiscussion(area, prompt) {
     ...
     area.innerHTML =
       ...
       prompt.students.map(s =>
         '<div class="student-response"><strong>' + s.name + ':</strong>' +
         '<p style="margin-top:6px;font-size:0.9rem;line-height:1.65;">' + s.response + '</p></div>'
       ).join('') +
       ...
       '<div class="word-count-bar"><span>Words: <span class="word-count-num" id="wc">0</span></span>' +
       '<span id="wcTarget" style="font-size:0.83rem;">Target: 120+ words</span></div>' +
       '<div style="display:flex;gap:10px;margin-top:16px;flex-wrap:wrap;">' +
       ...
       '</div></div>' +
       '<div id="discussFeedbackArea"></div>' +
     ...
     const update = () => {
       ...
       tgt.style.color = w >= 120 ? 'var(--success)' : 'var(--text-muted)';
       results[currentIdx] = { text: ta.value };
     };
     ta.addEventListener('input', update);
     update();
   ...
   function scoreDiscussion(idx) {
     ...
     if (text.trim().split(/\s+/).filter(Boolean).length < 20) {
       document.getElementById('discussFeedbackArea').innerHTML = '<div style="background:#fee2e2;border-left:4px solid #ef4444;border-radius:8px;padding:16px;margin-top:12px;font-size:0.9rem;">Please write your response first before requesting feedback.</div>';
       return;
     }
   ...
   function finishTask() {
     ...
     const bb = document.getElementById('backToTasksBtn');
     if (bb) bb.style.display = 'none';
     let correct = 0;
     if (currentTask === 'build-a-sentence') correct = results.filter(r => r?.correct).length;
     const sum = document.getElementById('scoreSummary');
     sum.style.display = 'block';
     sum.innerHTML = '<div class="score-summary"><h2>Task Complete!</h2>' +
